...
namespace libmpdataxx
{
  namespace opts
  {
    ...
    enum
    {
      ...
      fot = opts::bit(3), // fourth-order accuracy terms
      pfc = opts::bit(4), // use conditional statements like frac=where(den!=0,nom/den,0) instead of frac=nom/(den+eps) in psi-fraction factors 
      npa = opts::bit(5), // use nprt=(x-abs(x))/2 instead of nprt=min(0,x), and analogous formulae for pprt
      iga = opts::bit(6), // infinite-gauge option
      nug = opts::bit(7), // non-unit G (default G = 1) - see Smolarkiewicz 2006 eq (25) and discussion below for info on G
      dfl = opts::bit(8), // devergent flows
      khn = opts::bit(9) // use Kahan summation algorithm in the donor-cell formulae
    };
